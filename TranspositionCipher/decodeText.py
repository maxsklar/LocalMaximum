...
def decodeMessageTrialHillClimb(message, model, withDouble, useWordStrategy):
  charsInMessageList = list(set(message))
  charsInMessageList.sort()
  charsInMessage = ''.join(charsInMessageList)
  
  old_transpose = randomTranspose(len(charsInMessage))
  old_transformed = transformText(message, charsInMessage, old_transpose)
  old_entropy = languageModelLib.entropyOfTextGivenStrategy(old_transformed, model, useWordStrategy)
  ...
  while len(remainingTransposes) > 0:
    ...
    new_entropy = languageModelLib.entropyOfTextGivenStrategy(new_transformed, model, useWordStrategy)
    ...
    if (accept):
      ...
      if (withDouble and len(remainingTransposes) == 0) and (transposeStrategy == "Single"):
        transposeStrategy = "Double"
        remainingTransposes = generateAll2CharTransposes(old_transpose)
    
    #if (i % 1000 == 0):
    #  print "  ITER " + str(i), old_transformed, " -- ", ''.join(old_transpose), old_entropy, len(remainingTransposes)
    i += 1
  
  return old_transpose, old_transformed, old_entropy, i

def decodeMessageTrialMarkov(message, model, steps, useWordStrategy):
  charsInMessageList = list(set(message))
  charsInMessageList.sort()
  charsInMessage = ''.join(charsInMessageList)
...
# Possible searchStrategyValues
SEARCH_STRATEGY_HILL_CLIMB = 0
SEARCH_STRATEGY_HILL_CLIMB_WITH_DOUBLE = 1
SEARCH_STRATEGY_MARKOV_CHAIN = 1

def decodeMessage(message, model, strategy, useWordStrategy, trials):
  ...
  for trial in range(0, trials):
    result = None
    if (strategy == SEARCH_STRATEGY_HILL_CLIMB): result = decodeMessageTrialHillClimb(message, model, False, useWordStrategy)
    if (strategy == SEARCH_STRATEGY_HILL_CLIMB_WITH_DOUBLE): result = decodeMessageTrialHillClimb(message, model, True, useWordStrategy)
    if (strategy == SEARCH_STRATEGY_MARKOV_CHAIN): result = decodeMessageTrialMarkov(message, model, 1000, useWordStrategy)
